...
AutonManagerNode::AutonManagerNode(NodeManager* node_manager, IDriveNode* drive_node, ConveyorNode* conveyor_node, 
    ...
    m_programming_skills_auton = new ProgrammingSkillsAuton(drive_node, odometry_node, conveyor_node, intake_node, inertial_sensor_node);
    m_test_path_auton = new TestPathAuton(drive_node, odometry_node);
    m_test_pose_auton = new TestPoseAuton(drive_node, odometry_node);
