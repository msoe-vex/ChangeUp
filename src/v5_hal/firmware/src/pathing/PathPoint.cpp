...
PathPoint PathPoint::interpolateTo(PathPoint other, float time) {
    ...
    Eigen::Vector2d acceleration = deltaTime == 0. ? Eigen::Vector2d(0., 0.) : Eigen::Vector2d((other.getLinearVelocity().x() + getLinearVelocity().x()) / deltaTime, (other.getLinearVelocity().y(), getLinearVelocity().y()) / deltaTime);
