...
void SwerveController::assignActualAngle(int left_pot, int right_pot, int rear_pot) {
    ...
    m_rightActualMsg.data = right_actual_angle.angle();
    m_leftActualMsg.data = left_actual_angle.angle();
    m_rearActualMsg.data = rear_actual_angle.angle();
}
...
void SwerveController::periodic() {
    m_rightSetpointMsg.data = rightSwerveModule.getSetpoint().angle();
    m_leftSetpointMsg.data = leftSwerveModule.getSetpoint().angle();
    m_rearSetpointMsg.data = rearSwerveModule.getSetpoint().angle();
